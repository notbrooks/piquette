...
export const pinRouter = createTRPCRouter({
  ...
  delete: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await ctx.db.delete(pins).where(
        and(
          eq(pins.id, input.id),
          eq(pins.cuid, input.key),
          eq(pins.type, input.type),
        )
      );
  ...
  getByUser: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const pinsList = await ctx.db.select()
        .from(pins)
        .where(eq(pins.createdBy, input.createdBy))  // Corrected where clause
        .orderBy(pins.createdAt, desc(pins.createdAt))

      return pinsList ?? [];
    }),

  getAll: publicProcedure.query(async ({ ctx }) => {
    const pinsList = await ctx.db.query.pins.findMany({
      orderBy: (pins, { desc }) => [desc(pins.createdAt)],
    });

    return pinsList ?? null;
    }),
